...
def process_all_h5(directory='imorphics-cartilage', ram_per_worker_gb=2):
    """Process all .h5 files in the given directory using multiprocessing."""
    h5_files = [
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(concurrent.futures.as_completed(futures), total=len(h5_files), desc="Overall Progress"):
            result = future.result()
            results.append(result)
    return results

def read_h5_to_tensor(file_path, dataset_name):
    """Read .h5 data to PyTorch tensor."""
